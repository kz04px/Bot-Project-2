...
int buffer_fill_bot_body(s_part *part, s_buffers *buffers, float scale)
{
  ...
  // Vertices
  glBindBuffer(GL_ARRAY_BUFFER, buffers->vbo);
  glBufferData(GL_ARRAY_BUFFER, 2 * 5 * sizeof *bot_vertices, bot_vertices, GL_STATIC_DRAW);
  
  // Positions
  glBindBuffer(GL_ARRAY_BUFFER, buffers->tbo);
  glBufferData(GL_ARRAY_BUFFER, 2 * 5 * sizeof *bot_positions, bot_positions, GL_STATIC_DRAW);
  
  // Colours
  glBindBuffer(GL_ARRAY_BUFFER, buffers->cbo);
  glBufferData(GL_ARRAY_BUFFER, 3 * 5 * sizeof *bot_colours, bot_colours, GL_STATIC_DRAW);
  
  // Rotations
  glBindBuffer(GL_ARRAY_BUFFER, buffers->rbo);
  glBufferData(GL_ARRAY_BUFFER, 5 * sizeof *bot_rotations, bot_rotations, GL_STATIC_DRAW);
...
int buffer_fill_bot_spikes(s_part *part, s_buffers *buffers)
{
  ...
  // Vertices
  glBindBuffer(GL_ARRAY_BUFFER, buffers->vbo);
  glBufferData(GL_ARRAY_BUFFER, 2 * 3 * part->num_spikes * sizeof *spike_vertices, spike_vertices, GL_STATIC_DRAW);
  
  // Positions
  glBindBuffer(GL_ARRAY_BUFFER, buffers->tbo);
  glBufferData(GL_ARRAY_BUFFER, 2 * 3 * part->num_spikes * sizeof *spike_positions, spike_positions, GL_STATIC_DRAW);
  
  // Colours
  glBindBuffer(GL_ARRAY_BUFFER, buffers->cbo);
  glBufferData(GL_ARRAY_BUFFER, 3 * 3 * part->num_spikes * sizeof *spike_colours, spike_colours, GL_STATIC_DRAW);
  
  // Rotations
  glBindBuffer(GL_ARRAY_BUFFER, buffers->rbo);
  glBufferData(GL_ARRAY_BUFFER, 1 * 3 * part->num_spikes * sizeof *spike_rotations, spike_rotations, GL_STATIC_DRAW);
...
int buffer_fill_bot_eyes(s_part *part, s_buffers *buffers)
{
  ...
  // Vertices
  glBindBuffer(GL_ARRAY_BUFFER, buffers->vbo);
  glBufferData(GL_ARRAY_BUFFER, 2 * 4 * part->num_eyes * sizeof *eye_vertices, eye_vertices, GL_STATIC_DRAW);
  
  // Positions
  glBindBuffer(GL_ARRAY_BUFFER, buffers->tbo);
  glBufferData(GL_ARRAY_BUFFER, 2 * 4 * part->num_eyes * sizeof *eye_positions, eye_positions, GL_STATIC_DRAW);
  
  // Colours
  glBindBuffer(GL_ARRAY_BUFFER, buffers->cbo);
  glBufferData(GL_ARRAY_BUFFER, 3 * 4 * part->num_eyes * sizeof *eye_colours, eye_colours, GL_STATIC_DRAW);
  
  // Rotations
  glBindBuffer(GL_ARRAY_BUFFER, buffers->rbo);
  glBufferData(GL_ARRAY_BUFFER, 1 * 4 * part->num_eyes * sizeof *eye_rotations, eye_rotations, GL_STATIC_DRAW);
...
int buffer_fill_bot_cones(s_part *part, s_buffers *buffers)
{
  ...
  for(e = 0; e < part->num_eyes; ++e)
  {
    ...
    eye_cone_positions[4 * (EYE_CONE_ACCURACY + 2)*e + 4*(0) + 0] = part->x;
    eye_cone_positions[4 * (EYE_CONE_ACCURACY + 2)*e + 4*(0) + 1] = part->y;
    eye_cone_colours[6 * (EYE_CONE_ACCURACY + 2)*e + 6*(0) + 0] = part->eyes[e].r;
    eye_cone_colours[6 * (EYE_CONE_ACCURACY + 2)*e + 6*(0) + 1] = part->eyes[e].g;
    eye_cone_colours[6 * (EYE_CONE_ACCURACY + 2)*e + 6*(0) + 2] = part->eyes[e].b;
    eye_cone_rotations[2 * (EYE_CONE_ACCURACY + 2)*e + 2*(0) + 0] = 0.0;
    // p1
    eye_cone_vertices[4 * (EYE_CONE_ACCURACY + 2)*e + 4*(0) + 2] = 0.0;
    eye_cone_vertices[4 * (EYE_CONE_ACCURACY + 2)*e + 4*(0) + 3] = part->eyes[e].dist;;
    eye_cone_positions[4 * (EYE_CONE_ACCURACY + 2)*e + 4*(0) + 2] = part->x;
    eye_cone_positions[4 * (EYE_CONE_ACCURACY + 2)*e + 4*(0) + 3] = part->y;
    eye_cone_colours[6 * (EYE_CONE_ACCURACY + 2)*e + 6*(0) + 3] = part->eyes[e].r;
    eye_cone_colours[6 * (EYE_CONE_ACCURACY + 2)*e + 6*(0) + 4] = part->eyes[e].g;
    eye_cone_colours[6 * (EYE_CONE_ACCURACY + 2)*e + 6*(0) + 5] = part->eyes[e].b;
    eye_cone_rotations[2 * (EYE_CONE_ACCURACY + 2)*e + 2*(0) + 1] = part->angle + part->eyes[e].angle - part->eyes[e].fov/2;
    // p3
    eye_cone_vertices[4 * (EYE_CONE_ACCURACY + 2)*e + 4*(1) + 0] = 0.0;
    eye_cone_vertices[4 * (EYE_CONE_ACCURACY + 2)*e + 4*(1) + 1] = 0.0;
    eye_cone_positions[4 * (EYE_CONE_ACCURACY + 2)*e + 4*(1) + 0] = part->x;
    eye_cone_positions[4 * (EYE_CONE_ACCURACY + 2)*e + 4*(1) + 1] = part->y;
    eye_cone_colours[6 * (EYE_CONE_ACCURACY + 2)*e + 6*(1) + 0] = part->eyes[e].r;
    eye_cone_colours[6 * (EYE_CONE_ACCURACY + 2)*e + 6*(1) + 1] = part->eyes[e].g;
    eye_cone_colours[6 * (EYE_CONE_ACCURACY + 2)*e + 6*(1) + 2] = part->eyes[e].b;
    eye_cone_rotations[2 * (EYE_CONE_ACCURACY + 2)*e + 2*(1) + 0] = 0.0;
    // p4
    eye_cone_vertices[4 * (EYE_CONE_ACCURACY + 2)*e + 4*(1) + 2] = 0.0;
    eye_cone_vertices[4 * (EYE_CONE_ACCURACY + 2)*e + 4*(1) + 3] = part->eyes[e].dist;;
    eye_cone_positions[4 * (EYE_CONE_ACCURACY + 2)*e + 4*(1) + 2] = part->x;
    eye_cone_positions[4 * (EYE_CONE_ACCURACY + 2)*e + 4*(1) + 3] = part->y;
    eye_cone_colours[6 * (EYE_CONE_ACCURACY + 2)*e + 6*(1) + 3] = part->eyes[e].r;
    eye_cone_colours[6 * (EYE_CONE_ACCURACY + 2)*e + 6*(1) + 4] = part->eyes[e].g;
    eye_cone_colours[6 * (EYE_CONE_ACCURACY + 2)*e + 6*(1) + 5] = part->eyes[e].b;
    eye_cone_rotations[2 * (EYE_CONE_ACCURACY + 2)*e + 2*(1) + 1] = part->angle + part->eyes[e].angle + part->eyes[e].fov/2;
    ...
    for(n = 0; n < EYE_CONE_ACCURACY; ++n)
    {
      ...
      eye_cone_vertices[4 * (EYE_CONE_ACCURACY + 2)*e + 4*(n+2) + 1] = part->eyes[e].dist;
      eye_cone_positions[4 * (EYE_CONE_ACCURACY + 2)*e + 4*(n+2) + 0] = part->x;
      eye_cone_positions[4 * (EYE_CONE_ACCURACY + 2)*e + 4*(n+2) + 1] = part->y;
      eye_cone_colours[6 * (EYE_CONE_ACCURACY + 2)*e + 6*(n+2) + 0] = part->eyes[e].r;
      eye_cone_colours[6 * (EYE_CONE_ACCURACY + 2)*e + 6*(n+2) + 1] = part->eyes[e].g;
      eye_cone_colours[6 * (EYE_CONE_ACCURACY + 2)*e + 6*(n+2) + 2] = part->eyes[e].b;
      eye_cone_rotations[2 * (EYE_CONE_ACCURACY + 2)*e + 2*(n+2) + 0] = part->angle + part->eyes[e].angle - part->eyes[e].fov/2 + (float)n*(part->eyes[e].fov/EYE_CONE_ACCURACY);
      // pn+1
      eye_cone_vertices[4 * (EYE_CONE_ACCURACY + 2)*e + 4*(n+2) + 2] = 0.0;
      eye_cone_vertices[4 * (EYE_CONE_ACCURACY + 2)*e + 4*(n+2) + 3] = part->eyes[e].dist;
      eye_cone_positions[4 * (EYE_CONE_ACCURACY + 2)*e + 4*(n+2) + 2] = part->x;
      eye_cone_positions[4 * (EYE_CONE_ACCURACY + 2)*e + 4*(n+2) + 3] = part->y;
      eye_cone_colours[6 * (EYE_CONE_ACCURACY + 2)*e + 6*(n+2) + 3] = part->eyes[e].r;
      eye_cone_colours[6 * (EYE_CONE_ACCURACY + 2)*e + 6*(n+2) + 4] = part->eyes[e].g;
      eye_cone_colours[6 * (EYE_CONE_ACCURACY + 2)*e + 6*(n+2) + 5] = part->eyes[e].b;
      eye_cone_rotations[2 * (EYE_CONE_ACCURACY + 2)*e + 2*(n+2) + 1] = part->angle + part->eyes[e].angle - part->eyes[e].fov/2 + (float)(n+1)*(part->eyes[e].fov/EYE_CONE_ACCURACY);
    }
  }

  // Vertices
  glBindBuffer(GL_ARRAY_BUFFER, buffers->vbo);
  glBufferData(GL_ARRAY_BUFFER, 4 * (EYE_CONE_ACCURACY + 2) * part->num_eyes * sizeof *eye_cone_vertices, eye_cone_vertices, GL_STATIC_DRAW);

  // Positions
  glBindBuffer(GL_ARRAY_BUFFER, buffers->tbo);
  glBufferData(GL_ARRAY_BUFFER, 4 * (EYE_CONE_ACCURACY + 2) * part->num_eyes * sizeof *eye_cone_positions, eye_cone_positions, GL_STATIC_DRAW);

  // Colours
  glBindBuffer(GL_ARRAY_BUFFER, buffers->cbo);
  glBufferData(GL_ARRAY_BUFFER, 6 * (EYE_CONE_ACCURACY + 2) * part->num_eyes * sizeof *eye_cone_colours, eye_cone_colours, GL_STATIC_DRAW);

  // Rotations
  glBindBuffer(GL_ARRAY_BUFFER, buffers->rbo);
  glBufferData(GL_ARRAY_BUFFER, 2 * (EYE_CONE_ACCURACY + 2) * part->num_eyes * sizeof *eye_cone_rotations, eye_cone_rotations, GL_STATIC_DRAW);
